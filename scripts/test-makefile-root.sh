...
rm -f "$COMMAND_LOG"
if (cd "$CONTROL_DIR" && PATH="$CHECKOUT/bin:$PATH" REMIX_TUI_PROOF_COMMAND_LOG="$COMMAND_LOG" /usr/bin/make --no-print-directory --file "$EARLIER_MAKEFILE" --file "$MAKEFILE" check) >"$TEMP_ROOT/earlier-multiple.out" 2>&1; then
...
if (cd "$CONTROL_DIR" && PATH="$CHECKOUT/bin:$PATH" REMIX_TUI_PROOF_COMMAND_LOG="$COMMAND_LOG" /usr/bin/make --no-print-directory --file "$MAKEFILE" --file "$LATER_MAKEFILE" build) >"$TEMP_ROOT/later-multiple.out" 2>&1; then
